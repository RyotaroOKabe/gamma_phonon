...
df['gph'] = df['g_phs']#.map(lambda x: x.reshape((1, -1)))  #.map(lambda x: tosize(x, pad_size))

#%%
def append_nodes(struct):
    """_summary_
    ...
    # print('Original length: ', num_sites)
    struct2 = struct.copy()
    for i in range(2*num_sites):
        struct2.append(Atom('H', (0, 0, 0)))
    # print('Original length (2nd check): ', struct.get_positions().shape[0])
    # print('Updated length: ', struct2.get_positions().shape[0])
    return struct2

#%%
df['structure'] = df['structure_ase'].map(lambda x: append_nodes(x))
...
def build_data(entry, type_encoding, type_onehot, r_max=5.):
    ...
    edge_src, edge_dst, edge_shift = neighbor_list("ijS", a=entry.structure_ase, cutoff=r_max, self_interaction=True)

    num_sites = entry.sites #!
    idx_real = range(num_sites)
    idx_virt = range(num_sites, 3*num_sites)

    c = list(itertools.product(idx_real, idx_virt))
    edge_src2 = copy(edge_src)
    edge_dst2 = copy(edge_dst)
    edge_shift2 = copy(edge_shift)

    for i in range(len(c)):
        edge_src2 = np.append(edge_src2, np.array([c[i][0]]))
        edge_dst2 = np.append(edge_dst2, np.array([c[i][1]]))
        edge_shift2 = np.concatenate((edge_shift2, np.array([[0, 0, 0]])), axis=0)

    # print('num_sites: ', num_sites)
    # print('(real, virt) pairs: ', 2*num_sites**2)
    # print('src_old: ', edge_src.shape)
    # print('dst_old: ', edge_dst.shape)
    # print('shift_old: ', edge_shift.shape)
    # print('src_new: ', edge_src2.shape)
    # print('dst_new: ', edge_dst2.shape)
    # print('shift_new: ', edge_shift2.shape)

    # compute the relative distances and unit cell shifts from periodic boundaries
    edge_batch = positions.new_zeros(positions.shape[0], dtype=torch.long)[torch.from_numpy(edge_src2)]
    edge_vec = (positions[torch.from_numpy(edge_dst2)]
                - positions[torch.from_numpy(edge_src2)]
                + torch.einsum('ni,nij->nj', torch.tensor(edge_shift2, dtype=default_dtype), lattice[edge_batch]))

    # compute edge lengths (rounded only for plotting purposes)
    edge_len = np.around(edge_vec.norm(dim=1).numpy(), decimals=2)
    
    # print('edge_batch', edge_batch.shape)
    ...
    data = tg.data.Data(
        ...
        edge_index=torch.stack([torch.LongTensor(edge_src2), torch.LongTensor(edge_dst2)], dim=0),
        edge_shift=torch.tensor(edge_shift2, dtype=default_dtype),
        edge_vec=edge_vec, edge_len=edge_len,
        gph=torch.from_numpy(entry.gph.reshape((-1, 1))),#.unsqueeze(0),
        mpid=entry.id
    )
...
with torch.no_grad():
    ...
    for i, d in tqdm(enumerate(dataloader), total=len(dataloader), bar_format=bar_format):
        d.to(device)
        output = model(d)
        loss = F.mse_loss(output, d.gph.reshape((1, -1)), reduction='none').mean(dim=-1).cpu().numpy()   #!
        df.loc[i0:i0 + len(d.mpid) - 1, 'gph_pred'] = [[k] for k in output.cpu().numpy()]   #!
        df.loc[i0:i0 + len(d.mpid) - 1, 'mse'] = loss   #!
        i0 += len(d.mpid)
